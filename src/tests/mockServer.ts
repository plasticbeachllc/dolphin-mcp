...
export function startMockRest(port = 7777): Promise<() => Promise<void>> {
  ...
  const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
    try {
      ...
      if (
        req.method === "GET" &&
        (url.pathname.startsWith("/v1/chunks/") || url.pathname.startsWith("/chunks/"))
      ) {
        ...
        if (id === "not-found") {
          ...
          res.end(
            JSON.stringify({ error: { code: "chunk_not_found", message: "Chunk not found" } })
          );
          return;
        }
      ...
      if (req.method === "GET" && (url.pathname === "/v1/file" || url.pathname === "/file")) {
        ...
        if (start > end) {
          ...
          res.end(
            JSON.stringify({ error: { code: "invalid_range", message: "Invalid line range" } })
          );
          return;
        }
      ...
      if (req.method === "POST" && (url.pathname === "/v1/search" || url.pathname === "/search")) {
        ...
        if (repos && repos.some((r: string) => r.trim() === "nonexistent-repo")) {
          res.writeHead(404, { "Content-Type": "application/json" });
          res.end(
            JSON.stringify({ error: { code: "repo_not_found", message: "Repository not found" } })
          );
          return;
        }
