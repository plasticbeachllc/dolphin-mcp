...
export function makeGetVectorStoreInfo (): { definition: Tool, handler: any } {
  ...
  const handler = async (_args: any, signal?: AbortSignal): Promise<CallToolResult> => {
    ...
    try {
      ...
      await logInfo('get_vector_store_info', 'get_vector_store_info success', { latency_ms: Date.now() - started })
      return { content: [{ type: 'text', text: 'Vector store info ready.' }], isError: false, data }
    } catch (e: any) {
      const err = e?.error ? e : { error: { code: 'unexpected_error', message: e?.message ?? String(e) } }
      await logError('get_vector_store_info', 'get_vector_store_info error', { error_code: err.error.code, message: err.error.message })
      const content: CallToolResult['content'] = [{ type: 'text', text: `${err.error.message} Remediation: ensure REST service is running on 127.0.0.1:7777.` }]
